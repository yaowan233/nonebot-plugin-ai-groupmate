    ...
    *,
    ...
    async def schedule_message(
        ...
        try:
            ...
            logger.warning(
                "创建定时消息失败: "
                f"session={session_id}, error_type={type(error).__name__}"
            )
            return tool_failure(
                "schedule_failed",
                "定时消息创建失败。",
        ...
        return tool_success(
            "schedule_created",
            f"定时任务已创建，将在 {run_at.strftime('%Y-%m-%d %H:%M:%S')} 发送。",
            data={
                "job_id": job_id,
...
def create_schedule_agent_task_tool(
    ...
    *,
    ...
    async def schedule_agent_task(
        ...
        return tool_success(
            "schedule_created",
            f"定时 Agent 任务已创建，将在 {run_at.strftime('%Y-%m-%d %H:%M:%S')} 执行。",
            data={
                "job_id": job_id,
                "run_at": run_at.isoformat(),
                "task": task,
