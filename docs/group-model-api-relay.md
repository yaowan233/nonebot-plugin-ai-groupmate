...
- 私聊用户经同一配置码流程为自己配置主模型 API，无需群管理权限；
- 主聊天模型按会话解析和缓存失效：群聊会话查群级配置、私聊会话查用户级配置，未配置会话继续使用全局配置；
- 每群每日和每位私聊用户每日的公共主模型额度；
...
ticket_id
scope = group | private
group_id（私聊配置单存用户 ID，列名为数据库兼容保留）
operator_id
expires_at
created_at
```
...
/查看群API
/删除群API 确认
/配置个人API（别名：个人API配置、配置私聊API）
/提交个人API <配置码>（别名：提交API）
/查看个人API（别名：查看私聊API）
/删除个人API 确认（别名：删除私聊API）
```

所有命令通过正则 matcher 匹配，大小写不敏感（`API` 可写作 `api`、`Api` 等形式）。私聊命令只在私聊会话使用，操作者即本人，不需要群管理权限校验；配置单创建和配置码兑换的协议与群场景完全相同。
...
私聊场景对应每位用户的每日额度：`global_model_daily_private_user_limit_enabled` 默认开启，`global_model_daily_private_user_limit` 默认为 `10`，同样在配置中心的“基础行为”中热修改，语义与群额度一致（`0` 不限额、本地零点重置、一轮一次、超时保留计数）。额度耗尽的私聊提示附带 `/配置个人API` 指引和恢复时间。已保存并启用个人 API 的用户绕过公共额度。
...
| 操作者不匹配 | 请由发起配置的管理员本人提交（私聊场景：请由发起配置的用户本人提交） | 是 |
